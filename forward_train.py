...
from models.losses import ForwardDoctor, GradIntegral
...
def train_net(net,
              ...
              dir_checkpoint='details/checkpoints/ForwardDoctor',
    ...
    if args.model_name.lower() == 'resnet':
        ...
        if flag_3d:
            fd = ForwardDoctor(net,
                               [m for m in net.encoder.layer4.modules() if isinstance(m, nn.Conv2d)], 
            ...
            with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
                for imgs, true_categories, masks in train_loader:
                    ...
                    if net.n_classes > 1:
                        ...
                        loss_doctor = fd.loss(categories_pred, true_categories)
                    else:
                        pred_list += (categories_pred.detach().squeeze(1) > 0).float().tolist()
                        loss_cls = criterion(categories_pred, true_categories.unsqueeze(1))
                        loss_doctor = torch.tensor(0).to(device)
                    loss = loss_cls + loss_doctor

                    epoch_loss += loss.item() * imgs.size(0)
                    epoch_cls_loss += loss_cls.item() * imgs.size(0)
                    epoch_doctor_loss += loss_doctor.item() * imgs.size(0)
                    writer.add_scalar('Loss/train', loss.item(), global_step)
                    writer.add_scalar('Loss/cls_train', loss_cls.item(), global_step)
                    writer.add_scalar('Loss/doctor_train', loss_doctor.item(), global_step)

                    pbar.set_postfix(OrderedDict(**{'loss (batch)': loss.item(), 'cls':loss_cls.item(), 'doctor': loss_doctor.item()}))
...
if __name__ == '__main__':
    ...
    if args['method'].lower() == 'forward_doctor':
